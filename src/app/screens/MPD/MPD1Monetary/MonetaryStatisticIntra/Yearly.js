...
const Yearly = () => {
    ...
    useEffect(()=>{
        if(!visited && isFocus) {
            getMonetaryStatisticIntraReport_Y(report_type,date_type_default,year1,year2)
            setVisited(true);
        }
    },[isFocus])
    ...
    const SearchMonetaryStatisticIntraReport_Y = () =>{
        if(y2Status==true){
          if(checkSelectDateValidation(year1,year2,date_type).result){
            getMonetaryStatisticIntraReport_Y(report_type,date_type,year1,year2)
          }else{
              Toast.show({
                      type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchMonetaryStatisticIntraReport_Y}/>
            </View>
        </View>
