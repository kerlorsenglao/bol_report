...
const Monthly = () => {
    ...
    useEffect(()=>{
        getMonetaryStatisticIntraReport_M(report_type,date_type_default,my1,my2)
    },[])

    // function for MonthLy
    const getMonetaryStatisticIntraReport_M = async (report_type,date_type,my1,my2)=>{
    ...
    const SearchMonetaryStatisticIntraReport_M = () =>{
        if(my2status==true){
            if(checkSelectDateValidation(my1,my2,date_type).result){
                getMonetaryStatisticIntraReport_M(report_type,date_type,my1,my2)
            }else{
                Toast.show({
                    type: 'error',
                    text2: checkSelectDateValidation(my1,my2,date_type).msg,
                });
            }
        }else{
            getMonetaryStatisticIntraReport_M(report_type,date_type,my1,my2)
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchMonetaryStatisticIntraReport_M}/>
            </View>
