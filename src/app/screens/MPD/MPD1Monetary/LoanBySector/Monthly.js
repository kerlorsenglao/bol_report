...
const Monthly = () => {
    ...
    useEffect(()=>{
        getLoanBySectorReport_M(report_type,date_type_default,my1,my2)
    },[])
    ...
    const SearchLoanBySectorReport_M = () =>{
        if(my2status==true){
            if(checkSelectDateValidation(my1,my2,date_type).result){
                getLoanBySectorReport_M(report_type,date_type,my1,my2)
            }else{
                Toast.show({
                    type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchLoanBySectorReport_M}/>
            </View>
