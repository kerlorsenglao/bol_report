...
const API_NAME = "getMSLoanInterestRate"

// this function create by Toum at 21/12/2022
const LoanInterestScreen = () => {
    ...
    // useEffect
    useEffect(()=>{
      getLoanInterestReport_M(report_type,date_type_default,my1,my2)
    },[])

    // function for Yearly
    const getLoanInterestReport_M = async (report_type,date_type,my1,my2)=>{
        setIsLoading(true)
    ...
    const SearchLoanInterestReport_M = () =>{
        if(my2status==true){
            if(checkSelectDateValidation(my1,my2,date_type).result){
                getLoanInterestReport_M(report_type,date_type,my1,my2)
            }else{
                Toast.show({
                    type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchLoanInterestReport_M}/>
            </View>
        </View>
...
export default LoanInterestScreen