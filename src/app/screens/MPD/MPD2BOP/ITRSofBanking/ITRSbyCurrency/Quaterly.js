...
const API_NAME = "getITRSCurrencyReport"
...
const Quaterly = () => {
    ...
    useEffect(()=>{
      if(!visited && isFocus){
        getITRSbyCurrency_Q(report_type,date_type_default,t1,t2,y1,y2)
        setVisited(true)
      }
    },[isFocus])

    // function for Quaterly
    const getITRSbyCurrency_Q = async (report_type,date_type,t1,t2,y1,y2)=>{
        setIsLoading(true);
    ...
    const SearchITRSbyCurrency_Q = () =>{
        if(ty2Status==true){
            if(checkSelectDateValidation(y1,y2,date_type).result){
                getITRSbyCurrency_Q(report_type,date_type,t1,t2,y1,y2)
            }else{
                Toast.show({
                    type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchITRSbyCurrency_Q}/>
            </View>
        </View>
