...
const Yearly = () => {
    ...
    useEffect(()=>{
      if(!visited && isFocus) {
            getFDIbySectorReport_Y(report_type,date_type_default,year1,year2)
          setVisited(true)
      }
    },[isFocus])

    // function for Yearly
    const getFDIbySectorReport_Y = async (report_type,date_type,year1,year2)=>{
        setIsLoading(true)
    ...
    const SearchFDIbySectorReport_Y = () =>{
        if(y2Status==true){
            if(checkSelectDateValidation(year1,year2,date_type).result){
                getFDIbySectorReport_Y(report_type,date_type,year1,year2)
            }else{
                Toast.show({
                    type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchFDIbySectorReport_Y}/>
            </View>
        </View>
