...
const Monthly = () => {
    ...
    useEffect(()=>{
        if(!visited && isFocus){
            getITRSbyBank_M(report_type,date_type_default,my1,my2)
            setVisited(true)
        }
    },[isFocus])

    // function for MonthLy
    const getITRSbyBank_M = async (report_type,date_type,my1,my2)=>{
        setIsLoading(true)
  ...
  const SearchITRSbyBank_M = () =>{
      if(my2status==true){
        if(checkSelectDateValidation(my1,my2,date_type).result){
            getITRSbyBank_M(report_type,date_type,my1,my2)
        }else{
            Toast.show({
                    type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchITRSbyBank_M}/>
            </View>
        </View>
