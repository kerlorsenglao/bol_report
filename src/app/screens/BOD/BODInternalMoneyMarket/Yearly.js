...
const Yearly = () => {
    ...
    useEffect(()=>{
      if(!visited && isFocus) {
          getITRSbyBank_Y(report_type,date_type_default,year1,year2)
          setVisited(true)
      }
    },[isFocus])

    // function for Yearly
    const getITRSbyBank_Y = async (report_type,date_type,year1,year2)=>{
        setIsLoading(true)
    ...
    const SearchITRSbyBank_Y = () =>{
        if(y2Status==true){
            if(checkSelectDateValidation(year1,year2,date_type).result){
                getITRSbyBank_Y(report_type,date_type,year1,year2)
            }else{
                Toast.show({
                    type: 'error',
  ...
  return (
    <View style={{flex:1}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-evenly',paddingVertical: 5}}>
            ...
            <View style={{flex:1}}>
                <SearchButtonComponent searchFunction={SearchITRSbyBank_Y}/>
            </View>
        </View>
